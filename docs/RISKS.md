...
| 3 | Request latency exceeds API Gateway's 29s timeout | Medium | High | Scene cap (4/period) + concurrent reads: 116s to 18.6-26.1s across three separate live runs (variance is network-bound), measured on the canonical demo request (~25 km²). The slowest run is 90% of the 29s limit — see ADR 002. **Area matters much more than that figure suggests:** a hand-drawn ~47 km² area took ~200 s live (frontend session), and a concurrent live request failed at 51 s with GDAL "Read failed". The backend allows up to 2500 km² (`MAX_AOI_AREA_KM2`). The frontend now warns above ~30 km² and translates the 502, but the real remedies are the async worker (`TERRA_PROCESSING_MODE=async`, built, never run on real AWS) and a tighter live-mode AOI cap | Mitigated for the demo field, **open for larger areas** |
