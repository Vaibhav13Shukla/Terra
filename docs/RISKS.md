...
| 3 | Request latency exceeds API Gateway's 29s timeout | Medium | High | Scene cap (4/period) + concurrent reads: 116s to 18.6-26.1s across three separate live runs (variance is network-bound), measured on the canonical demo request. The slowest run is 90% of the 29s limit — see ADR 002 | Mitigated, not eliminated |
